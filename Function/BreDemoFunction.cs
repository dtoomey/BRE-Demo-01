...
namespace INTEGRATE2024
{
    ...
    /// <summary>
    /// Represents the BreDemoFunction flow invoked function.
    /// </summary>
    public class BreDemoFunction
    {
        private readonly ILogger<BreDemoFunction> logger;

        public BreDemoFunction(ILoggerFactory loggerFactory)
        {
            logger = loggerFactory.CreateLogger<BreDemoFunction>();
        }
            ...
            string zipCode)
        {
            ...
            try
            {
                ...
                XmlNamespaceManager nsmgr = new XmlNamespaceManager(updatedDoc.NameTable);
                nsmgr.AddNamespace("ns0", "http://BizTalk_Server_Project1.SchemaUser");

                // Apply the discount
                Single.TryParse(updatedDoc.DocumentElement.SelectSingleNode("//Status/Discount", nsmgr).InnerText, out float discount);
                currentPurchase.PurchaseAmount -= (currentPurchase.PurchaseAmount * (discount/100));

                var ruleExectionOutput = new RuleExecutionResult()
                {
                    XmlDoc = updatedDoc.OuterXml,
                    DiscountedPurchaseAmount = currentPurchase.PurchaseAmount,
        ...
        public class RuleExecutionResult
        {
            ...
            /// <summary>
            /// Purchase amount with discount applied
            /// </summary>
            public float DiscountedPurchaseAmount { get; set;}
            ...
            /// </summary>
            public float PurchaseAmountPostTax { get; set;}
        }
    }
}